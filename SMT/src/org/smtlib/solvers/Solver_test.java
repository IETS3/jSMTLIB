...
public class Solver_test implements ISolver {
	...
	public IResponse get_assertions() {
		...
		if (!smtConfig.relax && !Utils.TRUE.equals(get_option(smtConfig.exprFactory.keyword(Utils.PRODUCE_ASSERTIONS)))) {
			...
			return smtConfig.responseFactory.error("jSMTLIB: The get-assertions command is only valid if " + key + " has been enabled");
		}
	...
	public IResponse check_sat_assuming(IExpr ... exprs) {
		...
		if (logicSet == null) {
			return smtConfig.responseFactory.error("jSMTLIB: The logic must be set before a check-sat-assuming command is issued");
		}
		for (IExpr e: exprs) {
			if (e instanceof IFcnExpr && ((IFcnExpr)e).args().size() != 0) {
				IFcnExpr f = (IFcnExpr)e;
				if (f.args().size() != 1 || !f.head().toString().equals("not")) {
					return smtConfig.responseFactory.error("jSMTLIB: Each element of a check-sat-assuming command must be either p or (not p), where p is a Bool constant");
				}
				e = f.args().get(0);
			}
			if (!(e instanceof IIdentifier)) {
				return smtConfig.responseFactory.error("jSMTLIB: Expected a simple identifier: " + e); // FIXME - use pretty printer?
			}
			...
			if (entries.size() != 1) {
				return smtConfig.responseFactory.error("jSMTLIB: No zero-arity declaration of symbol " + e); // FIXME - use pretty printer?
			}
			if (!entries.get(0).sort.isBool()) {
				return smtConfig.responseFactory.error("jSMTLIB: Expected a Bool symbol: " + e + " has sort " + entries.get(0).sort); // FIXME - use pretty printer?
			}
		}
	...
	public IResponse get_model() {
		...
		if (checkSatStatus == smtConfig.responseFactory.unsat()) {
			return smtConfig.responseFactory.error("jSMTLIB: The get-model command is only valid immediately after check-sat returned sat or unknown");
		}
		return smtConfig.responseFactory.unsupported();
	}
	...
	public IResponse pop(int number) {
		...
		if (assertionSetStack.size() <= number) {
			return smtConfig.responseFactory.error("jSMTLIB: The argument to a pop command is too large: " + number + " vs. a maximum of " + (assertionSetStack.size()-1));
		} else {
			while (--number >= 0) {
...
//			lit = smtConfig.exprFactory.unquotedString(value);
	...
	public IResponse declare_const(Ideclare_const cmd) {
		if (logicSet == null) {
			return smtConfig.responseFactory.error("jSMTLIB: The logic must be set before a declare-const command is issued");// FIXME - position and on other similar statements
		}
		...
		if (list.isEmpty()) {
			...
			if (symTable.add(entry,false)) { 
				...
				return smtConfig.responseFactory.error("jSMTLIB: Symbol " + encodedName + " is already defined",cmd.symbol().pos());
			}
		} else {
	...
	public IResponse declare_fun(Ideclare_fun cmd) {
		if (logicSet == null) {
			return smtConfig.responseFactory.error("jSMTLIB: The logic must be set before a declare-fun command is issued");// FIXME - position and on other similar statements
		}
		String encodedName = encode(cmd.symbol());
	...
	public IResponse declare_sort(Ideclare_sort cmd) {
		...
		if (b) {
			...
			if (!b) return smtConfig.responseFactory.error("jSMTLIB: The identifier is already declared to be a sort: " + 
